...
{
	#[inline(always)]
	pub(crate) fn is_slice<ReentryPoint, R>(&mut self, compare_to: &[u8], reentry_point: ReentryPoint, invalid_reason: InvalidReason, result: R) -> Result<R, Status<ReentryPoint>>
	{
		...
		{
			Err(RanOutOfBytes(reentry_point))
		}
	}
