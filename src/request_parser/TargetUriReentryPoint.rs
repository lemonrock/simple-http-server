...
pub(crate) enum TargetUriReentryPoint
{
	Begin(NonNull<u8>),
...
impl TargetUriReentryPoint
{
	...
	pub(crate) fn parse(bytes: &mut Bytes, target_uri_user: &mut impl TargetUriUser) -> Result<(), Status<Self>>
	{
		Self::parse_start(bytes, target_uri_user)
	}

	pub(crate) fn re_enter(self, bytes: &mut Bytes, target_uri_user: &mut impl TargetUriUser) -> Result<(), Status<Self>>
	{
		use self::Status::*;
		...
		match self
		{
			...
			SegmentStartsFrom(uri_starts_at_inclusive, start_at) =>
			{
				...
				Self::parse_segments(uri_starts_at_inclusive, bytes, target_uri_user)
			}
			...
			QueryStartsFrom(uri_starts_at_inclusive, start_at) =>
			{
				...
				Self::parse_query(uri_starts_at_inclusive, bytes, target_uri_user)
			}
		}
	}
	...
	#[inline(always)]
	fn parse_start(bytes: &mut Bytes, target_uri_user: &mut impl TargetUriUser) -> Result<(), Status<Self>>
	{
		...
		{
			b'/' => Self::parse_segments(uri_starts_at_inclusive, bytes, target_uri_user),
	...
	#[inline(always)]
	fn parse_segments(uri_starts_at_inclusive: NonNull<u8>, bytes: &mut Bytes, target_uri_user: &mut impl TargetUriUser) -> Result<(), Status<Self>>
	{
		...
		loop
		{
			match bytes.if_has_more_return_current_value_and_increment(reentry_point)?
			{
				b' ' =>
				{
					target_uri_user.use_segment(reentry_point.0, bytes.curent_pointer).map_err(|invalid_reason| Invalid(invalid_reason))?;
					return target_uri_user.finished().map_err(|invalid_reason| Invalid(invalid_reason))
				}
				...
				{
					target_uri_user.use_segment(reentry_point.0, bytes.curent_pointer).map_err(|invalid_reason| Invalid(invalid_reason))?;
					return Self::parse_query(uri_starts_at_inclusive, bytes, target_uri_user)
				}
	...
	#[inline(always)]
	fn parse_query(uri_starts_at_inclusive: NonNull<u8>, bytes: &mut Bytes, target_uri_user: &mut impl TargetUriUser) -> Result<(), Status<Self>>
	{
		...
		loop
		{
			match bytes.if_has_more_return_current_value_and_increment(reentry_point)?
			{
				b' ' =>
				{
					target_uri_user.use_query(reentry_point.0, bytes.curent_pointer).map_err(|invalid_reason| Invalid(invalid_reason))?;
					return target_uri_user.finished().map_err(|invalid_reason| Invalid(invalid_reason))
				},
